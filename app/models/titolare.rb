class Titolare < ActiveRecord::Base
  ...
  def unique_entry
    matched_entry = Titolare.where(['LOWER(nome) = LOWER(?) AND LOWER(cognome) = LOWER(?) AND LOWER(telefono) = LOWER(?) AND LOWER(cf) = LOWER(?) AND data_nascita=?',
       self.nome, self.cognome, self.telefono, self.cf, self.data_nascita]).first #il '?' e' un parametro per SQL passato da self.campo
